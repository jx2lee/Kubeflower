...
function get_resource_knative(){
    ns=knative-serving
    declare -a rscList=("bindings" "configmaps" "endpoints" "events" "limitranges" "persistentvolumeclaims" "pods" "podtemplates" "replicationcontrollers" "resourcequotas" "secrets" "serviceaccounts" "services" "controllerrevisions" "daemonsets" "deployments" "replicasets" "statefulsets" "localsubjectaccessreviews" "horizontalpodautoscalers" "cronjobs" "jobs" "cephblockpools" "cephclusters" "cephfilesystems" "cephnfses" "cephobjectstores" "cephobjectstoreusers" "leases" "events" "daemonsets" "deployments" "ingresses" "networkpolicies" "replicasets" "alertmanagers" "podmonitors" "prometheuses" "prometheusrules" "servicemonitors"  "poddisruptionbudgets" "rolebindings" "roles" "volumes" "volumesnapshots")
    
    for rsc in ${rscList[*]}
    do
    	echo "[$rsc]"
    	kubectl get $rsc -n $ns
      echo ""
    done
}


function get_resource_kubeflow(){
    ns=kubeflow
    declare -a rscList=("componentstatuses" "namespaces" "nodes" "persistentvolumes" "mutatingwebhookconfigurations" "validatingwebhookconfigurations" "customresourcedefinitions" "apiservices" "tokenreviews" "selfsubjectaccessreviews" "selfsubjectrulesreviews" "subjectaccessreviews" "certificatesigningrequests" "podsecuritypolicies" "runtimeclasses" "podsecuritypolicies" "clusterrolebindings" "clusterroles" "priorityclasses" "volumesnapshotclasses" "volumesnapshotcontents" "csidrivers" "csinodes" "storageclasses" "volumeattachments")
    
    for rsc in ${rscList[*]}
    do
    	echo "[$rsc]"
    	kubectl get $rsc | grep $ns
    	echo ""
    done
}


function delete_resource_kubeflow(){
...
function main() {
  case "${1:-}" in
  get_knative)
    get_resource_knative
    ;;
  get_kubeflow)
    get_resource_kubeflow
    ;;
  rm_kubeflow)
    delete_resource_kubeflow
    ;;
  rm_namespace)
    delete_namespace
    ;;
  *)
    set +x
    echo "usage:" >&2
    echo "    $0 get_knative" >&2
    echo "    $0 get_resource_kubeflow" >&2
    echo "    $0 rm_kubeflow" >&2
    echo "    $0 rm_namespace" >&2
    ;;
  esac
}
